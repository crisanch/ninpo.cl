...
global $post, $woocommerce, $product, $dfd_ronneby;
...
<div class="images columns <?php echo esc_attr($woo_image_section_class) ?>">
	<div class="single-product-image woocommerce-product-gallery__image">
		...
		if ( has_post_thumbnail() ) {
			...
			$thumb_link  = wp_get_attachment_image_src( $id, 'thumbnail' );
			...
			echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<div itemprop="image"  id="'. esc_attr($unique_id) .'" class="woocommerce-main-image %s">%s <a href="'.esc_url($image_link).'" class="prettyPhoto" data-rel="prettyPhoto[woo_single_gal]" data-thumb="%s"><img src="%s" alt="" class="dfd-large-image" /></a></div>', $image_title, $image, $thumb_link[0], $image_link ), $post->ID );
			?>
