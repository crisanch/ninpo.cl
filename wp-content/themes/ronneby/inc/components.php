 ...
 * @return string Post like code
 */
function getPostLikeLinkOld($post_id=null) {
	if (!$post_id) {
		global $post;
    ...
    if(hasAlreadyVoted($post_id)) {
        $output .= '<i class="dfd-icon-heart"></i><span title="'.esc_html__('I like this article', 'dfd').'" class="like alreadyvoted"><span class="count">'.esc_html__($vote_count).'</span></span>';
	} else {
        $output .= '<a class="post-like" href="#" data-post_id="'.esc_attr($post_id).'">
