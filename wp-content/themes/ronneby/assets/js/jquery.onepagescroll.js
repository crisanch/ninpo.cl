...
//				scrollbarWidth = typeof dfd_native != 'undefined' && typeof dfd_native.scrollbarWidth != 'undefined' ? dfd_native.scrollbarWidth : 0;
			...
			if($('#wpadminbar').length > 0 && !$('body').hasClass('compose-mode') && $(window).width() > 640) {
				offset += $('#wpadminbar').outerHeight();
			}
			
			if($('.dfd-frame-line.line-top').length > 0 && $(window).width() > 640) {
...
//					element.find('.fp-scroller').css('height',contentHeight + headerHeight);
					element.find('.fp-scroller').css('height',contentHeight);
...
//								}
	...
	if($container.length > 0) {
		if($headerContainer.length > 0 && $container.data('smart-header') && $container.data('smart-header') == '1' && ($headerContainer.hasClass('header-style-2')
			|| $headerContainer.hasClass('header-style-4')
			|| $headerContainer.hasClass('header-style-7'))) {
			$headerContainer.addClass('dfd-smart-header');
			smartHeader = true;
			lightLogo = dfd_smart_logo_light;
			darkLogo = dfd_smart_logo_dark;
		}
